...
class ProductCrossSellingEvent implements EventSubscriberInterface
{
    ...
    {
        return [
            ProductCrossSellingsLoadedEvent::class => ['onCrossellingSlider', 50]
        ];
    }
    ...
    public function onCrossellingSlider(ProductCrossSellingsLoadedEvent $event)
    {
        $context = $event->getSalesChannelContext();
        $crossSellingElements = $event->getCrossSellings()->getElements();

        // Read out variants from elements
        $masterProducts = array();

        foreach ($crossSellingElements as $crossSellingElement) {
            $products = $crossSellingElement->getProducts()->getElements();
            ...
            foreach ($products as $key => $product) {
                ...
                // Reset entities
                $crossSellingElement->getProducts()->remove($key);

                // Refill entities with masterProduct Values
                $crossSellingElement->getProducts()->set($key, $masterProduct);
            }
        }
    }
}
